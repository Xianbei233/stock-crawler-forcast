let color_list = ['#c23531', '#2f4554', '#61a0a8', '#d48265', '#91c7ae', '#749f83', '#ca8622', '#bda29a', '#6e7074', '#546570', '#c4ccd3'];
let colors = ['#5793f3', '#d14a61', '#675bba', '#b62f46'];
let close = GOOGLE['data'].map(function (el, idx) {
  return el[1];
})
let stocks = GOOGLE['data'].map(function (el, idx) {
  return [el[0], el[1], el[3], el[2]];
})
let stock_date = GOOGLE['date'];
let volume = GOOGLE['volume'];

let indeces = {};
let dataMA5, dataMA10, dataMA20, dataMA30;
let total_investment, total_gain, stock_changes, stock_changes_percent
...
function completeFn(results) {
  ...
  csv = results['data'];
  console.log(csv)
  for (let i = 0; i < csv[0].length; i++) indeces[csv[0][i].toLowerCase()] = i;
  stocks = [];
  volume = [];
  stock_date = [];
...
function calculate_distribution(real, predict) {
  ...
  for (let outer = 0; outer < data_arr.length; outer++) {
    ...
    for (let i = 0; i < bar_y.length; i++) bar_y[i] -= min_line_y
  ...
  let bar_plot = echarts.init(document.getElementById('div_dist')); //echarts框架初始化
...
function plot_stock() {
  ...
  let chart_stock = echarts.init(document.getElementById('div_output'));
...
$('#trainbutton').click(function () {
  ...
  setTimeout(function () {
    ...
    function async_training_loop(callback) {
      (function loop(i) {
        let total_loss = 0
        for (let k = 0; k < Math.floor(X_scaled.shape[0] / timestamp) * timestamp; k += timestamp) {
        ...
        $('#log').append('Epoch: ' + (i + 1) + ', 平均损失: ' + total_loss + '<br>');
        ...
        let chart_line = echarts.init(document.getElementById('div_loss'));
    ...
    async_training_loop(function () {
      ...
      $('#table-body').html('');
      for (let i = 0; i < my_investment['output'].length; i++) $('#table-body').append(my_investment['output'][i]);
      $('#log-invest').append("<h6 class='header'>总共赚取: " + my_investment['overall gain'] + ", 总投资率: " + my_investment['overall investment'] + "%</h5>")
      ...
      let markpoints = []
      for (let i = 0; i < my_investment['buy_X'].length; i++) {
        ind = new_date.indexOf(my_investment['buy_X'][i])
        markpoints.push({ name: '买入', value: '买入', xAxis: ind, yAxis: my_investment['buy_Y'][i], itemStyle: { color: '#61a0a8' } })
      }
      for (let i = 0; i < my_investment['sell_X'].length; i++) {
        ind = new_date.indexOf(my_investment['sell_X'][i])
        markpoints.push({ name: '卖出', value: '卖出', xAxis: ind, yAxis: my_investment['sell_Y'][i], itemStyle: { color: '#c23531' } })
      }
      option = {
        animation: false,
        color: color_list,
...
function plot_pairplot(val) {
  let chart = echarts.init(document.getElementById('pairplot'));
  let columns = ['Close', 'Crude Oil', 'Diesel', 'Gasoline', 'Gold', 'Heating Oil', 'Kerosene', 'Natural Gas', 'Propane'];
  ...
  let chart_pi = echarts.init(document.getElementById('pi_correlation'));
  let seriesData = [];
  let selected = {};
  for (let i = 0; i < val['pi'].length; i++) {
  ...
  let chart_pie = echarts.init(document.getElementById('div_pie'));
  ...
  titles = [];
  count = 0;
  for (let i = 0; i < val['movement_changes'].length; i++) {
    let data = [];
    for (let k = 0; k < val['movement_changes'][i]['movement'].length; k++) {
  ...
  rowNumber = Math.ceil(Math.sqrt(count));
  echarts.util.each(grids, function (grid, idx) {
  ...
  let chart_changes = echarts.init(document.getElementById('percent_changes'));
